...
# 13 "../../firmware/user_uart.h"
extern int UART_BUF[8];
# 3 "uart.c" 2
...
void __attribute__ ( ( section ( ".mprj" ) ) ) uart_write(int *n)
{
    for (int i = 0; i < 8; i++){
        ...
        if (*(n+i) != 0)
            (*(volatile uint32_t*)0x30000004) = *(n+i);
        else
            break;
    }
}
...
{
    int j = 0;
    while (((((*(volatile uint32_t*)0x30000008)) & 1) == 0) && (j < 8)){
        for(int i = 0; i < 1; i++)
            asm volatile ("nop");

        UART_BUF[j] = (*(volatile uint32_t*)0x30000000);
        j++;
    }
    while (j < 8){
        UART_BUF[j] = 0;
