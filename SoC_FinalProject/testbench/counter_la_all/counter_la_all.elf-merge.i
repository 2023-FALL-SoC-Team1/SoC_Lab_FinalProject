# 0 "merge.c"
# 1 "/home/ubuntu/SoC-Lab-WLOS-main/testbench/wlos//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "merge.c"
# 1 "fir.h" 1
...
int* __attribute__ ( ( section ( ".mprjram" ) ) ) qsort(){
 volatile int32_t* ptr = &(*(volatile uint32_t*)0x30000280);

 while(!(((*(volatile uint32_t*)0x30000200) &(0x00000004)) == 0x00000004));
 (*(volatile uint32_t*)0x30000200) = 0x00000001;

 for(int i=0; i< 10; i++){
  *(ptr) = Q[i];
 }

 while(!(((*(volatile uint32_t*)0x30000200) &(0x00000002)) == 0x00000002));

 ptr = &(*(volatile uint32_t*)0x30000284);
 for (int i=0; i<10; i++){
  Q[i] = *(ptr);
 }
 return Q;
...
{
 volatile int32_t* ptr = &(*(volatile int32_t*)0x30000184);


 while(!(((*(volatile uint32_t*)0x30000100) &(0x00000004)) == 0x00000004));
 (*(volatile uint32_t*)0x30000100) = 0x00000001;
 for(int i=0; i< 4; i++){
  *(ptr) = B[i*4];
  *(ptr) = B[i*4 +1];
  *(ptr) = B[i*4 +2];
 ...
 ptr = &(*(volatile int32_t*)0x30000180);
 for(int i=0; i< 4; i++){
  *(ptr) = A[i*4];
  *(ptr) = A[i*4 +1];
  *(ptr) = A[i*4 +2];
 ...
 ptr = &(*(volatile int32_t*)0x3000018C);
 for (int i=0; i<4; i++){
  result[i*4] = *(ptr);
  result[i*4 +1] = *(ptr);
  result[i*4 +2] = *(ptr);
...
int* __attribute__ ( ( section ( ".mprjram" ) ) ) fir(){
 ...
 volatile int32_t* ptr = ((volatile int32_t*)0x30000040);

 for(int i=0; i< 11; i++){
  *(ptr + i) = taps[i];
 }

 while(!(((*(volatile uint32_t*)0x30000000) &(0x00000002)) == 0x00000000));
 (*(volatile uint32_t*)0x30000000) = 0x00000001;


 int fir_output;
 (*(volatile uint32_t*)0x30000080) = 1;
 for(int i = 2; i <= 64; i = i + 1) {
     fir_output = (*(volatile uint32_t*)0x30000084);
